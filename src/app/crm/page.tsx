...
export default async function CRMPage() {
    ...
    return (<div className="p-6">
            ...
            {customers.length === 0 ? (<p className="text-gray-500 italic">No customers found.</p>) : (
                <table className="min-w-full border border-gray-300 text-sm">
                    ...
                    {customers.map((customer) => (<CustomerRow
                            key={customer.id}
                            customer={customer}
                            interactions={interactions.filter(interaction => interaction.customer_id === customer.id)}
                        />))}
                    </tbody>
                </table>)}
        </div>);
