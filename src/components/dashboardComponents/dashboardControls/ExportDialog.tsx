...
const exportSchema = z.object({
    exportCustomers: z.boolean(), exportInteractions: z.boolean(), exportTransactions: z.boolean(),
});
...
export function ExportDialog({customers, interactions, transactions}: ExportDialogProps) {
    const form = useForm<ExportValues>({
        resolver: zodResolver(exportSchema), defaultValues: {
            exportCustomers: true, exportInteractions: true, exportTransactions: true,
        },
    });
        ...
        <DialogContent>
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    {(['Customers', 'Interactions', 'Transactions'] as const).map((label, i) => (<FormField
                        key={label}
